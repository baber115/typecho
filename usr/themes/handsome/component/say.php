...
$GLOBALS['request_url'] = $this->request->getRequestUrl();
        ...
        <div  class="time-machine m-l-lg panel box-shadow-wrap-normal">
            <div class="panel-heading pos-rlt b-b b-light">
                ...
                <span class="text-muted m-l-sm pull-right"
                      datetime="<?php $comments->date('c'); ?>"><a href="#<?php $comments->theId()?>"><?php echo Utils::formatDate($comments, $comments->created, $options->dateFormat); ?></a>
                </span>
            </div>
            <div class="panel-body comment-content-true" id="<?php $comments->theId() ?>-body" data-id="<?php $comments->theId() ?>" data-comment="<?php
            $comment = array(
                'text'      =>  $comments->text
            );
            echo htmlspecialchars(Json::encode($comment)); ?>" data-edit_action="<?php Utils::GetTokenUrl('/action/cross-edit?do=edit&coid=' . $comments->coid);?>">
                <!--                <span class="comment-author-at"><b>-->
                <?php //get_comment_at($comments->coid)?><!--</b></span>-->
                <?php echo CommentContent::postCommentContent(ScodeContent::parseContentPublic($comments->content),
                    $GLOBALS['isLogin'], "", "", "", true); ?>
            </div>
            <div class="panel-footer" id="<?php $comments->theId() ?>-footer" data-id="<?php $comments->theId() ?>">
                <div class="say_footer bg-white-pure">
                    ...
                    <?php  if ($GLOBALS['isLogin']): ?>
                        <span data-id="<?php $comments->theId() ?>" data-delete_action="<?php Utils::GetTokenUrl('/action/cross-edit?do=delete&coid=' . $comments->coid) ?>" class="time_operation say_footer_right dropdown">
...
<div id="comments" class="timeMachine">
    ...
    no-borders border-radius-6">
            ...
            <div class="panel panel-default m-t-md pos-rlt m-b-md no-borders border-radius-6">
                <form id="comment_form" action="<?php $this->commentUrl() ?>" method="post" class="comment-form"
                      role="form">
                    <input type="hidden" name="receiveMail" id="receiveMail" value="yes" />

                    <textarea id="comment" class="OwO-textarea border-radius-6-top textarea form-control no-border" name="text" rows="3"
                          maxlength="65525" aria-required="true"><?php
                        if ($tid){
                            echo TagContent::getTagName($tid, "", true) . " ";
                        }
                    ...
                    <div <?php if (!$this->user->hasLogin()) echo 'style="padding-left: 5px"'; ?> class="panel-footer
                     bg-light lter border-radius-6-bottom">
                        ...
                        <div class="">
                            ...
                            <button type="submit" name="submit" id="submit" class="submit box-shadow-wrap-lg btn-gd-primary pull-right btn-sm">
            ...
            <i class="animate-spin fontello fontello-spinner hide" id="spin"></i>
                            ...
                            <ul class="nav nav-pills nav-sm" id="time_toolbar">
                                <li class="OwO" style="width: 38px;height: 32px"></li>
...
<!--        <h4>--><?php //_me("此处评论已关闭") ?><!--</h4>-->
    ...
    <?php
    $tag_name = TagContent::getTagName($tid);
    if ($tag_name) {
        echo '<a class="bg-tag-a" data-pjax href="?"><span class="bg-tag tag-close lg dk">'.$tag_name.'<i data-feather="x-square" class="close_icon"></i></span></a>';
    }
    ?>
    ...
    <?php else: ?>
        ...
        <div class="streamline b-l m-l-lg m-b padder-v">
            <ol class="comment-list list-unstyled m-b-none">
                <div id="comment-0" class="comment-body comment-parent comment-odd comment-by-author">

                    <a class="pull-left thumb-sm avatar m-l-n-md">
                        <img nogallery src="<?php $this->options->BlogPic(); ?>" class="img-40px photo img-square normal-shadow">
                    </a>
                    ...
                    <div class="m-l-lg m-b-md">
                        ...
                        <div class="m-b">
                            <div class="m-b">
                                ...
                                <b><a class="highlightlink" href="https://auth.ihewro.com/user/docs/#/FAQ/comment"><?php _me("关于时光机你最可能遇到的问题，请查看使用文档有关「时光机」常见问题") ?></a></b>
                                <?php else: ?>
                                    <p><?php _me("当前标签下没有时光机的记录，请重新筛选试试") ?></p>
                                <?php  endif;?>
                            </div>
                        </div>
                    </div>
...
<?php
if (Utils::getExpertValue("time_first_letter",false)):?>
<style>
